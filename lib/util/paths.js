...
// Fallbacks for other operating systems
    ...
    paths.config = paths.config || path.join(base, '.config/upt');
    paths.data = paths.data || path.join(base, '.local/share/upt');
    paths.cache = paths.cache || path.join(base, '.cache/upt');
}

paths.tmp = path.resolve(path.join(tmp, 'upt'));
